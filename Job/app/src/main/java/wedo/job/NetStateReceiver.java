...
import wedo.job.impl.NetChangeObserver;
...
public class NetStateReceiver extends BroadcastReceiver {
    ...
    private static NetStateUtil.NetType mNetType;
    //监听者列表
    private static ArrayList<NetChangeObserver> mNetChangeObservers = new ArrayList<NetChangeObserver>();
    private static BroadcastReceiver mBroadcastReceiver;

    public final static String CUSTOM_ANDROID_NET_CHANGE_ACTION = "wedo.job.receiver.CONNECTIVITY_CHANGE";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION) || intent.getAction().equalsIgnoreCase(CUSTOM_ANDROID_NET_CHANGE_ACTION)) {
            if (!NetStateUtil.isNetworkAvailable(context)) {
                ...
                isNetAvailable = true;
                mNetType = NetStateUtil.getAPNType(context);
            }
            notifyObserver();
        }
    }
    ...
    public static NetStateUtil.NetType getAPNType() {
        return mNetType;
    }
